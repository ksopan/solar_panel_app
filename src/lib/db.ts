...
export function getDatabase(env?: any) {
  ...
  if (env?.DB) {
    console.log('✅ Using provided D1 DB');
  ...
  // Development: use SQLite if file exists
  if (process.env.NODE_ENV === 'development') {
    console.log('🔍 NODE_ENV=development');
    console.log('🔍 Checking for SQLite DB at:', DB_PATH);
    ...
    console.warn('⚠️ SQLite DB not found — falling back to mock DB');
  }

  // Mock fallback
  console.warn('⚠️ Using mock DB');
  return {
    prepare: () => ({
      bind: () => ({
        first: async () => null,
