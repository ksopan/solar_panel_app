...
export function getDatabase(env?: any) {
  ...
  // 🌍 globalThis for edge or Cloudflare
  if (typeof globalThis !== 'undefined' && 'DB' in globalThis) {
    console.log('Using DB from globalThis context');
    return drizzleD1((globalThis as any).DB);
  }

  // 💻 Development mode - use local SQLite
  if (process.env.NODE_ENV === 'development') {
    ...
    if (fs.existsSync(DB_PATH)) {
      ...
      console.warn('❌ SQLite DB not found. Path:', DB_PATH);
    }
  }

  // 🧪 Fallback to mock DB (you can copy your mockDB code here if you still want it)
  console.warn('⚠️ Using fallback mock DB');
  return {
    prepare: () => ({
      bind: () => ({
        first: async () => null,
